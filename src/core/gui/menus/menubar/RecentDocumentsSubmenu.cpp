...
void openFileCallback(GSimpleAction* ga, GVariant* parameter, Control* control) {
    size_t l = 0;
    const char* p = g_variant_get_string(parameter, &l);
    std::string s(p, l);

    auto path = Util::fromUri(s);
    if (path) {
        control->openFile(path.value());
    }
}
...
auto createRecentMenuItem(const GtkRecentInfo* info, size_t i) {
    ...
    std::string label = FS(FORMAT_STR("{1}. {2}") % i % display_name);

    std::string action = RecentDocumentsSubmenu::G_ACTION_NAMESPACE;
    action += RecentDocumentsSubmenu::OPEN_ACTION_NAME;
    action += "('";
    action += gtk_recent_info_get_uri(const_cast<GtkRecentInfo*>(info));
    action += "')";
...
RecentDocumentsSubmenu::RecentDocumentsSubmenu(Control* control, GtkApplicationWindow* win) {
    ...
    if (!win) {
        ...
        openFileAction.reset(g_simple_action_new(OPEN_ACTION_NAME, G_VARIANT_TYPE_STRING), xoj::util::adopt);
        g_signal_connect(G_OBJECT(openFileAction.get()), "activate", G_CALLBACK(openFileCallback), control);
