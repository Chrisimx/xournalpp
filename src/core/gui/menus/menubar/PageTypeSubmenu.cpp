...
auto createPageTypesSection(const std::vector<std::unique_ptr<PageTypeInfo>>& pageTypes, size_t index) {
    ...
    std::string actionName = PageTypeSubmenu::G_ACTION_NAMESPACE;
    actionName += PageTypeSubmenu::SELECTION_ACTION_NAME;
    ...
    for (auto& pageInfo: pageTypes) {
        ...
        g_menu_item_set_action_and_target_value(item.get(), actionName.c_str(), g_variant_new_uint64(index++));
...
auto createApplyToAllPagesSection() {
    ...
    // Todo(c++20) constexpr this
    std::string actionName = PageTypeSubmenu::G_ACTION_NAMESPACE;
    actionName += PageTypeSubmenu::APPLY_ALL_ACTION_NAME;
