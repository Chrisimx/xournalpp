...
namespace {
size_t find_index_of(const std::vector<std::unique_ptr<PageTypeInfo>>& types, const PageType& pt) {
    auto it = std::find_if(types.begin(), types.end(), [&pt](const auto& typeinfo) { return typeinfo->page == pt; });
...
GSimpleAction* createPageTypeSelectionAction(PageTypeHandler* types, const std::optional<PageType>& pt,
    ...
    size_t index = PageTypeSelectionMenuBase::findIndex(types, pt);
    return g_simple_action_new_stateful(actionName.data(), G_VARIANT_TYPE_UINT64, g_variant_new_uint64(index));
}
};  // namespace


size_t PageTypeSelectionMenuBase::findIndex(PageTypeHandler* types, const std::optional<PageType>& pt) {
...
void PageTypeSelectionMenuBase::setSelectedPageType(const std::optional<PageType>& selected) {
