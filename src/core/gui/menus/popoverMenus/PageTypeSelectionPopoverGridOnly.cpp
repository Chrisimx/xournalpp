...
GtkWidget* PageTypeSelectionPopoverGridOnly::createPreviewGrid(
        const std::vector<std::unique_ptr<PageTypeInfo>>& pageTypes, const std::string_view& prefixedActionName) {
    /*
     * Todo(gtk4): replace the GtkMenu with a GtkPopover, using PageTypeSelectionPopover::createPreviewGrid instead.
     * Note that GtkPopover is no good with GTK 3, as the popover gets cropped to the dialog window (so part of is not
     * displayed). GtkMenus don't have this restriction but are removed in GTK 4. Popovers in GTK 4 do not have this
     * restriction.
     */
    ...
    for (const auto& pageInfo: pageTypes) {
        ...
        gtk_actionable_set_action_target_value(GTK_ACTIONABLE(entry), g_variant_new_uint64(n++));
        gtk_container_add(GTK_CONTAINER(entry), createPreviewImage(*pageInfo));
